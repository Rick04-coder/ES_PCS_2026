...
int main() {
	...
	for (int k = 0; k < 100; k++)
	{
		...
		if (!is_sorted(vi))
		{
			cerr << "Selectionsort fallito su vettore int" << endl;
			return EXIT_FAILURE;
		}
		if (!is_sorted(vf))
		{
			cerr << "Selectionsort fallito su vettore float" << endl;
			return EXIT_FAILURE;
		}
		if (!is_sorted(vd))
		{
			cerr << "Selectionsort fallito su vettore double" << endl;
			return EXIT_FAILURE;
		}
	}
	...
	if (!is_sorted(vs))
	{
		cerr << "Selectionsort fallito su vettore stringhe" << endl;
		return EXIT_FAILURE;
	}
	cout << "Selectionsort: tutti i test superati" << endl;
	return EXIT_SUCCESS;
}